...
import { GuestbookQueryParams, GuestbookListResponse } from "@/types/guestbooks/guestbookTypes";


export const useGetGuestbookList = (params: GuestbookQueryParams = {}) => {
    return useQuery({
    ...
    // gcTime: 1000 * 60 * 5,    // cacheTime 대신 gcTime 사용(컴포넌트가 언마운트되더라도 5분 동안은 캐시를 유지)
        queryFn: async () => {
            ...
            const response = await fetchClient(`/guestbooks?${queryString}`, {skipAuth: true});
            return response.json() as Promise<GuestbookListResponse>;
        }
    });
};