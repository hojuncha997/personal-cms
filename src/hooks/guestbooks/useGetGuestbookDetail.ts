// src/hooks/guestbooks/useGetGuestbookDetail.ts

import { useQuery, useQueryClient } from "@tanstack/react-query";
import { fetchClient } from "@/lib/fetchClient";
        // import { PostDetailResponse } from "@/types/posts/postTypes";
import { GuestbookDetail } from "@/types/guestbooks/guestbookTypes";
import { useAuthStore } from "@/store/useAuthStore";

export const useGetGuestbookDetail = (public_id: string, options?: {enabled?: boolean}) => {
    const { isAuthenticated } = useAuthStore();
    
    return useQuery<GuestbookDetail>({
        ...
        queryFn: async () => {
            ...
            const data = await fetchClient(`/guestbooks/${public_id}`, requestConfig);
            
            // .json()은 기본적으로 any타입을 반환한다. any에 직접 타입 캐스팅을 하는 것은 안전하지 않을 수 있다.
            // 따라서 먼저 unknown으로 타입 추론 후 그 다음 타입 캐스팅을 적용하였다.
            const guestbookData = await data.json();
            return guestbookData as unknown as GuestbookDetail;
        },
