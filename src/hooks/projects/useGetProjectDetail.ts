...
import { ProjectDetail } from "@/types/projects/projectTypes";
import { useAuthStore } from "@/store/useAuthStore";

export const useGetProjectDetail = (public_id: string, options?: {enabled?: boolean}) => {
    const { isAuthenticated } = useAuthStore();
    
    return useQuery<ProjectDetail>({
        queryKey: ['projects', public_id],
        queryFn: async () => {
            // 로그인 여부에 따라 토큰 사용 여부 결정. 로그인 한 경우 토큰 전송(회원 확인용)
            const requestConfig = isAuthenticated ? {skipAuth: false} : {skipAuth: true};
            const data = await fetchClient(`/projects/${public_id}`, requestConfig);
            ...
            return projectData as unknown as ProjectDetail;
        },
