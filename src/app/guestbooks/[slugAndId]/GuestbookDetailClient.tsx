// src/app/guestbooks/[slugAndId]/GuestbookDetailClient.tsx
// 방명록 상세 페이지
...
const GuestbookDetailClient: React.FC<GuestbookDetailClientProps> = ({ publicId }) => {
    const { data: guestbook, isLoading, error } = useGetGuestbookDetail(publicId);
    ...
    useEffect(() => {
        if (guestbook) {
            incrementViewCount({ contentType: 'guestbooks', publicId });
        }
    }, [guestbook, publicId, incrementViewCount]);

    const handleDeleteGuestbook = async () => {
        if (window.confirm('정말로 이 방명록을 삭제하시겠습니까?')) {
            try {
                await deleteGuestbook(publicId);
                        ...
                        <div className="bg-white rounded-lg sm:border-none">
                            <div className="p-0 sm:p-6">
                                ...
                                <div className="border-b border-gray-300 pb-4">
                                    <div className="mb-2">
                                        <span className="px-2 py-1 bg-gray-100 text-xs rounded-md">{guestbook.categorySlug || 'no category'}</span>
                                    </div>
                                    <h1 className="text-xl sm:text-2xl font-bold mb-2">{guestbook.title}</h1>
                                    <div className="flex justify-between items-center">
                                        <div className="flex flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span>{guestbook.author_display_name?.includes('@') ? guestbook.author_display_name.split('@')[0] : guestbook.author_display_name}</span>
                                            </div>
                                            <span>조회수: {guestbook.viewCount}</span>
                                            <span>작성일: {format(new Date(guestbook.createdAt), 'yyyy-MM-dd')}</span>
                                        </div>
                                        {isAuthor && (
                                            <CommonPopover
                                                trigger={
                                            ...
                                            >
                                                <div className="py-1 text-black">
                                                    <button 
                                                        ...
                                                        onClick={() => router.push(`/guestbooks/${guestbook.slug}-${publicId}/edit`)}
                                                    >
                                                        수정하기
                                                    </button>
                                                    <button 
                                ...
                                <div className="mt-8 border-t border-gray-300 pt-4">
                                    <div className="flex justify-center mb-8">
                                        ...
                                        >
                                            ...
                                            <span>좋아요 {guestbook.likeCount + (isLiked ? 1 : 0)}</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
...
export default GuestbookDetailClient; 